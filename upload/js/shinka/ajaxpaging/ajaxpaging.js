...
XF.ShinkaAjaxPage = XF.Element.newHandler({
    ...
    /**
     * Splits given replace string into the selector to replace and the selector to replace with
     */
    splitParts: function()
    {
    ...
    /**
     * Sends AJAX request to the given href
     */
    ...
    /**
     * Disables buttons and pagers to prevent duplicate requests
     */
    ...
    /**
     * Re-enables buttons after request is processed
     */
    ...
    updateUrl: function()
    {
        history.pushState({page: this.page, href: this.href},
            document.title,
            this.href);
    },

    /**
     * Updates document title with current page and XF phrase
     * @param title Container title, e.g. subject of a thread
     */
    updateTitle: function(title)
    {
        document.title = this.page > 1 ?
            title + XF.phrase('title_page_x', {'{page}': this.page}) + ' | Xenforo' :
            title + ' | Xenforo';
    },
    ...
    handleResponse: function($html, container)
    {
        ...
        $animateOut.xfFadeUp(null, function()
        {
            ...
            $new = self.options.animate ? $new.filter(`:not(${self.options.animate})`) : $new;
            $old.remove();
